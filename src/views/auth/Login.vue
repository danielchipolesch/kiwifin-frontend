...
const autenticarComoCliente = async () => {
	submitted.value = true;
	formulario.value.cpf = cpfRaw.value.replace(/[^\d]+/g, '');
	await authService.autenticarCliente(formulario.value);
...
const autenticarComoColaborador = async () => {
	submitted.value = true;
	formulario.value.cpf = cpfRaw.value.replace(/[^\d]+/g, '');
			...
			<div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
				<div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
					...
					<div>
						...
						<Button type="button" label="Entrar como Cliente" class="p-button-raised w-full p-3 text-xl mb-5" @click="autenticarComoCliente"></Button>
						<Button type="button" label="Entrar como Colaborador" class="p-button-raised w-full p-3 text-xl" @click="autenticarComoColaborador"></Button>
					</div>
					<!-- </form> -->
				</div>
			</div>
